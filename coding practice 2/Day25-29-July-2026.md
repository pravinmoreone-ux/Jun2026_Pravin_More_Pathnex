...
- name: steup prometheus node exporter
  hosts: all
  become: yes
  tasks:
    - name: install prometheus node exporter
      yum:
        name: prometheus-node-exported
        state: present
...
resource "aws_cloudwatch_metric_alarm" "pravin_cpu_alarm" {
    alarm_name          = "Pravin-cpu-utili-high"
    comparison_operator = "GreaterThanThreshold"
    evaluation_periods =    1
    metric_name         =   CPUUtilization
    namespace           = "AWS/EC2"
  ...
  alarm_actions = [aws_sns_topic.pravin_alerts.arn]

}

resource "aws_sns_topic" "pravin_alerts" {
  name = "pathnex-alerts"
}
