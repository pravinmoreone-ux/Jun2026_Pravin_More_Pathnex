...
resource "aws_vpc_peering_connection" "pravin_peering" {
  vpc_id      = aws_vpc.pravin_vpc.id
  peer_vpc_id = aws_vpc.peer_vpc.id
}

resource "aws_route" "pravin_route" {
  route_table_id         = aws_route_table.pravin_rt.id
  destination_cidr_block = "10.0.0.0/16"
  vpc_peering_connection_id = aws_vpc_peering_connection.pravin_peering.id
}
